...
export default function DJPanel({ songs, onAddCalledSong, onOpenWinners, onExit, spotify }) {
  ...
  const [hookOffsets, setHookOffsets] = useState({});
  ...
  function handleNext() {
    ...
    spotify.pausePlayback();
    if (currentSong) {
      onAddCalledSong(currentSong.name);
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1200, background: COLORS.navy, display: 'flex', color: 'white', fontFamily: 'sans-serif' }}>
      <div style={{ width: '220px', borderRight: '1px solid rgba(255,255,255,0.1)', display: 'flex', flexDirection: 'column', paddingTop: '60px', flexShrink: 0 }}>
        <div style={{ padding: '12px 16px', fontSize: '11px', fontWeight: '700', letterSpacing: '1.5px', textTransform: 'uppercase', color: 'rgba(255,255,255,0.45)', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
          Played · {playedSongs.length}
        </div>
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {playedSongs.length === 0 ? (
            <div style={{ padding: '20px 16px', fontSize: '12px', color: 'rgba(255,255,255,0.22)', textAlign: 'center' }}>No songs played yet</div>
          ) : (
            [...playedSongs].reverse().map((song, i) => (
              <div key={i} style={{ padding: '9px 16px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                <div style={{ fontSize: '13px', fontWeight: '600', lineHeight: 1.3 }}>{song.name}</div>
                {song.artist && <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', marginTop: '2px' }}>{song.artist}</div>}
              </div>
            ))
          )}
        </div>
